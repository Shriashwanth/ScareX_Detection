    ...
    def _listen_loop(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                # Check RMS volume level to ignore silence/background hum
                rms = float(np.sqrt(np.mean(np.square(audio_data))))
                min_rms = float(self.config.get('audio_min_rms', 0.02))
                if rms < min_rms:
                    self.last_detected_bird = "No Bird Detected"
                    self.last_confidence = 0.0
                    continue
                ...
                if features is not None:
                    ...
                    audio_threshold = self.config.get('audio_confidence', 0.75)
                    if confidence >= audio_threshold:
                        if max_prob_index < len(self.labels):
                            predicted_label = self.labels[max_prob_index]
                            if predicted_label.lower() in ["background", "unknown"]:
                                self.last_detected_bird = "No Bird Detected"
                                self.last_confidence = 0.0
                            else:
